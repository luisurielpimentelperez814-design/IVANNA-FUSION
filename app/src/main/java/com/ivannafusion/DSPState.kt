...
object DSPState {
    ...
    // ── Hardware real del dispositivo (NO 192kHz/24-bit fijo) ─────────────────
    // Consultado real vía AudioManager.PROPERTY_OUTPUT_SAMPLE_RATE/
    // FRAMES_PER_BUFFER en detectRealHardwareCapabilities(). La mayoría
    // de teléfonos Android reportan 48000 Hz en su path de salida
    // estándar — 192kHz/24-bit reales requieren un DAC USB-C externo
    // con su propio driver, no el codec interno. deviceSupportsHighRes
    // solo es true si el valor REAL reportado es >= 88200 Hz (umbral
    // estándar de la industria para hi-res audio); nunca se fuerza.
    var deviceSampleRateHz     by mutableIntStateOf(48000)
    var deviceFramesPerBuffer  by mutableIntStateOf(0)
    var deviceSupportsHighRes  by mutableStateOf(false)
    ...
    fun detectRealHardwareCapabilities(context: Context) {
        try {
            ...
            deviceSampleRateHz = sr
            deviceFramesPerBuffer = fpb
            deviceSupportsHighRes = sr >= 88200
        } catch (e: Exception) {
